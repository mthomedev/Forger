<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <p class="subtitle">Join the community of creators and makers.</p>
      ...
      <form @submit.prevent="handleRegister" class="auth-form" novalidate>
        <div class="input-group">
          ...
          <input v-model="form.name" placeholder="Full Name" required class="auth-input" autocomplete="name" />
        </div>
        <div class="input-group">
          <span class="input-icon" aria-hidden="true">
          ...
          <input v-model="form.email" type="email" placeholder="Email address" required class="auth-input" autocomplete="email" />
        </div>
        <div class="input-group">
          <span class="input-icon" aria-hidden="true">
          ...
          <input v-model="form.password" type="password" placeholder="Password" required class="auth-input" autocomplete="new-password" />
        </div>
        <div class="input-group">
          <span class="input-icon" aria-hidden="true">
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/><path d="M12 15v2"/></svg>
          </span>
          <input v-model="form.password_confirmation" type="password" placeholder="Confirm Password" required class="auth-input" autocomplete="new-password" />
        </div>
        ...
        <p class="terms">By signing up, you agree to our Terms, Privacy Policy and Maker Guidelines.</p>

        <button type="submit" class="auth-btn" :disabled="authStore.loading">
          <span v-if="authStore.loading" class="spinner"></span>
          <span>{{ authStore.loading ? 'Igniting Workshop...' : 'Sign Up' }}</span>
        </button>
      </form>
    </div>
...
const handleRegister = async () => {
  try {
    ...
    errorMessage.value = err.message || err.errors?.email?.[0] || 'Registration failed'
  }
}
</script>
